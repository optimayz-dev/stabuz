                                        ...
                                        <p class="text-muted font-13 m-b-30">
                                            ...
                                            <ul class="nav navbar-right panel_toolbox">
                                                ...
                                                <li class="dropdown" style="padding-right: 15px;">
                                                    ...
                                                    <div class="dropdown-menu" aria-labelledby="dropdownMenuButton">
                                                        <a class="dropdown-item" href="{{ route('admin.promotion.create') }}">Создать</a>
                                                    </div>
                                                </li>
                                            </ul>
                                    </div>
...
{{--                                                </td>--}}
                                                <td>{{ $item->id }}</td>
                                                <td><a href="{{ route('admin.promotion.edit', $item->id) }}">{{ $item->title }}</a></td>
                                                <td>{{ $item->seo_title }}</td>
                                                <td>{{ Str::limit($item->seo_description, 50) }}</td>
                                                <td>{{ Str::limit($item->meta_keywords, 50) }}</td>
                                                <td style="text-align: center"><img src="{{ asset($item->image ?? '') }}" alt="" style="width: 80px"></td>
                                                <td>{{ $item->created_at }}</td>
                                                <td style="display: flex; justify-content: center;">
                                                    <a href="{{ route('admin.promotion.edit', $item->id) }}" class="btn btn-outline-primary">
                                                    ...
                                                    <form action="{{ route('admin.promotion.destroy', $item->id) }}" method="POST">
                                                        @csrf
                                                        @method('delete')
